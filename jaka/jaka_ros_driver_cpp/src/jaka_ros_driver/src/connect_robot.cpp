...
//
    ...
    robot.kine_forward(&joint_pose, &cartesian_pose);
    geometry_msgs::PoseStamped pose;
    pose.pose.position.x = cartesian_pose.tran.x/1000; // mm to m 
    pose.pose.position.y = cartesian_pose.tran.y/1000; //
    pose.pose.position.z = cartesian_pose.tran.z/1000; //
    
    RotMatrix matrix;
    Quaternion qua;
    robot.rpy_to_rot_matrix(&(cartesian_pose.rpy), &matrix);
    robot.rot_matrix_to_quaternion(&matrix, &qua);
    
    pose.pose.orientation.w=qua.s;
...
void* get_conn_scoket_state(void* args){
    ...
    while (ros::ok())
    {
        ...
        // for (int i = 0; i < 6; i++)
        // {
        //     cout<<"postition:"<<robot_status.joint_position[i]<<endl;
        // }
...
int main(int argc, char **argv)
{
    ...
    arm_pose_pub = n.advertise<geometry_msgs::PoseStamped>("/robot_driver/arm_pose", 10);
    ...
    while (ret_status.enabled == false)
    { 
        ROS_INFO("restart......");
        robot.power_off();
        sleep(1);
