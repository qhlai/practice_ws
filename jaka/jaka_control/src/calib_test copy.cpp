...
Eigen::Quaterniond  rpy2qua(double rx, double ry, double rz){
    ...
    Eigen::Quaterniond quaternion = rotation_x * rotation_y * rotation_z;
...
// }
    ...
    tmp=set_pose(-0.15,-0.6,0.3,PI*0.75, PI*0.25, PI*-0.25);


    // Eigen::Vector3d direction(0, -1, 0);
    // Eigen::AngleAxisd rotation_vector(M_PI/10, direction);
    // Eigen::Quaterniond quat(rotation_vector);  // 将 AngleAxisd 转换为 Quaterniond
    // tmp.orientation.w = quat.w();
    // tmp.orientation.x = quat.x();
    // tmp.orientation.y = quat.y();
    // tmp.orientation.z = quat.z();
    ...
    {
        Eigen::Vector3d direction(0, -1, 0);
        tmp=set_pose(-0.15,-0.6,0.3, direction);
    }
    move_group.setPoseTarget(tmp);
    arm_move(move_group, my_plan);
    sleep(3);
    ...
    {
        Eigen::Vector3d direction(-1, -1, 0);
        tmp=set_pose(-0.15,-0.6,0.3, direction);
    }
    move_group.setPoseTarget(tmp);
    arm_move(move_group, my_plan);
    ...
    {
        Eigen::Vector3d direction(1, -1, -1);
        tmp=set_pose(-0.15,-0.6,0.3, direction);
    }
    move_group.setPoseTarget(tmp);
    arm_move(move_group, my_plan);
    ...
    {
        Eigen::Vector3d direction(1, 0, -1);
        tmp=set_pose(-0.15,-0.6,0.3, direction);
    }
    move_group.setPoseTarget(tmp);
    arm_move(move_group, my_plan);
